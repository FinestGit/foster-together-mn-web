...
import { AgencyCreateInputSchema, AgencyParseError, parseAgency, parseAgencyList } from "./agency"
...
describe('parseAgency', () => {
    ...
    it("throws AgencyParseError when shape is wrong", () => {
        ...
        expect(() => parseAgency({ ...validAgency, id: "string" })).toThrow(AgencyParseError)
        vi.restoreAllMocks()
    })
...
describe('parseAgencyList', () => {
    ...
    it("throws when not an array", () => {
        vi.spyOn(console, 'error').mockImplementation(() => {})
        expect(() => parseAgencyList({ ...validAgency, id: 1 })).toThrow(AgencyParseError)
        vi.restoreAllMocks()
    })

    it("throws when element is invalid", () => {
        vi.spyOn(console, 'error').mockImplementation(() => {})
        expect(() => parseAgencyList([{ ...validAgency, id: "string" }])).toThrow(AgencyParseError)
        vi.restoreAllMocks()
    })
})
